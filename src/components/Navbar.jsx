...
const Navbar = () => {
    ...
    return (
        ...
        <div className={`navbar bg-base-100 static h-20 rounded-xl my-0.5 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
            <div className="navbar-start">
                <div className="dropdown">
                    <label tabIndex={0} className="btn btn-ghost lg:hidden">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                    </label>
                    <ul tabIndex={0} className="menu  dropdown-content mt-3 z-20 p-2 shadow bg-base-100 text-base-100 rounded-box w-52 ">
                        {Links}
                    </ul>
                </div>
                <Link to='/' > <img className="w-36" src="https://i.ibb.co/M2Rnc8M/logo.png" alt="" /></Link>
            </div>
                    ...
                    <div className="dropdown dropdown-end z-20">
                        <label tabIndex={0} className="btn btn-ghost btn-circle avatar">
                            <div className="w-10 rounded-full">
                                <img src={user?.photoURL || "https://i.ibb.co/CWJcyPj/omar-faruk.jpg"} alt="User Avatar" />
                            </div>
                        </label>
                        <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 ">
                            <li>
                                <button className="btn btn-sm  btn-ghost">{user?.displayName || "Md Omar Faruk"}</button>
                            </li>
                            <li>
